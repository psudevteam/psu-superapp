    ...
    utils.lib.eachDefaultSystem (system:
      let
        ...
        createEmulator = pkgs:
          ...
          in rec {
            ...
            abi.type = arch;
            packages = "system-images;android-${
                toString androidPlatformsVersion
              };${tag.id};${abi.type}";
            sysdir = "$ANDROID_SDK_ROOT/system-images/android-${
                toString androidPlatformsVersion
              }/${tag.id}/${abi.type}/";
        ...
        overlays = [
          ...
          (final: prev: rec {
            ...
            androidSdk = let emulator = createEmulator prev;
            in prev.androidSdk (s: [
              ...
              s."${replace.underscoreToDash
              (replace.semicolonToDash emulator.packages)}"
            ]);
          })
        ];
        ...
        scripts = let emulator = createEmulator pkgs;
        in with pkgs; [
          # help commands
          ...
          (writeScriptBin "check-react-native" ''
            until ${run "yarn"} react-native -v > /dev/null 2>&1; do
              ...
              ${
                run "yarn"
              } install || (echo failed instll dependencies with yarn; exit 1)
              echo ""
      ...
      in {
        devShells.default = pkgs.mkShell {
          ...
          ANDROID_NDK_ROOT =
            "${pkgs.androidSdk.outPath}/share/android-sdk/ndk/${androidNDKVersion}";
