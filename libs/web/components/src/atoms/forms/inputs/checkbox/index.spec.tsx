...
describe('Test InputCheckbox Functionality', () => {
  ...
  it('Should be not checked', () => {
    const { getByTestId } = render(
      <InputCheckbox checked={false} defaultChecked={false} />
    );
    fireEvent.click(getByTestId('input-checkbox'));
  ...
  it('Should be disabled', () => {
    const { getByTestId } = render(<InputCheckbox disabled />);
    fireEvent.click(getByTestId('input-checkbox'));
    expect(getByTestId('input-checkbox')).toBeDisabled();
  });

  it('Should be not disabled', () => {
    const { getByTestId } = render(<InputCheckbox disabled={false} />);
    fireEvent.click(getByTestId('input-checkbox'));
    expect(getByTestId('input-checkbox')).not.toBeDisabled();
  });

  it('Should be required', () => {
    const { getByTestId } = render(<InputCheckbox required />);
    fireEvent.click(getByTestId('input-checkbox'));
    expect(getByTestId('input-checkbox')).toBeRequired();
  });

  it('Should be not required', () => {
    const { getByTestId } = render(<InputCheckbox required={false} />);
    fireEvent.click(getByTestId('input-checkbox'));
  ...
  it('Should have id', () => {
    const { getByTestId } = render(<InputCheckbox name="test" />);
  ...
  it('Should have value', () => {
    const { getByTestId } = render(<InputCheckbox value="test" />);
    expect(getByTestId('input-checkbox')).toHaveAttribute('value', 'test');
  });

  it('Should called onChange', () => {
    const onChange = vi.fn();
    const { getByTestId } = render(<InputCheckbox onChange={onChange} />);
    fireEvent.click(getByTestId('input-checkbox'));
